...
- **按键正确/错误的绿红箭头全是裂图** —— `keyImage()` 拼状态后缀时漏了下划线，
  运行时请求 `UPG.webp` / `aR.webp`，而实际文件是 `UP_G.webp` / `a_R.webp`，
  于是每次按键反馈都变成破图。这是「比赛画面异常」的又一表现。
  现统一在 `keyImage()` 内规范化状态后缀（`''` / `'G'` / `'_G'` 都可）。
...
`Image`，加载**真实的** ES Module 并驱动完整游戏流程。覆盖：13 个界面的进入与交互、
